window.onload = lightsensing4();

function lightsensing4(){
	setInterval(function(){ //wait function 
		...
		if (request.readyState == 4 && request.status == 200){ //server status and checks 
				...
				document.getElementById("light4").innerHTML =  request.responseText; //document is assigning the value from above into the table 
			}
		}
		request.open('POST', 'lightsensorvalue4.php', true); //actually opening and sending the files 
		request.send();
	},1000);
}	//wait
