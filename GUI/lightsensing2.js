window.onload = lightsensing2();

function lightsensing2(){
	setInterval(function(){ //wait function 
		...
		if (request.readyState == 4 && request.status == 200){ //server status and checks 
				...
				document.getElementById("light2").innerHTML =  request.responseText; //document is assigning the value from above into the table 
			}
		}
		request.open('POST', 'lightsensorvalue2.php', true); //actually opening and sending the files 
		request.send();
	},1000);
}	//wait
